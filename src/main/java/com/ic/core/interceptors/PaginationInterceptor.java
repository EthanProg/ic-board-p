package com.ic.core.interceptors;
...
/**
 * 功能：mybatis物理分页拦截器，支持'mysql'、'oracle'、'db2'、'mssql12'、'mssql58'
 * @author 郑斌	2014年8月1日 上午11:24:57
 * 修改说明：
 */
...
public class PaginationInterceptor implements Interceptor{
	...
	public void setProperties(Properties p) {
		//根据配置的dialect属性，来确定使用的dialect实例
		String dialectPreperty = p.getProperty("dialect");
        if (dialectPreperty ==null || dialectPreperty.equals("")) {
            try {
        	...
        	if(dialectPreperty.toUpperCase().equals("MYSQL")){
        		dialect = dialect = new MySql5Dialect();
        	}else if(dialectPreperty.toUpperCase().equals("ORACLE")){
        		dialect = dialect = new OracleDialect();
        	}else if(dialectPreperty.toUpperCase().equals("DB2")){
        		dialect = dialect = new DB2Dialect();
        	}else if(dialectPreperty.toUpperCase().equals("MSSQL12")){
        		dialect = dialect = new Mssql12Dialect();
        	}else if(dialectPreperty.toUpperCase().equals("MSSQL58")){
        		dialect = dialect = new Mssql58Dialect();
        	}else{
        		try {
                    throw new PropertyException("dialect only support 'mysql' or 'oracle' or 'db2' or 'mssql12' or 'mssql58'!");
                } catch (PropertyException e) {
                    e.printStackTrace();
                }
        	}
